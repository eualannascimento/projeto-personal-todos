...
## 0. Stack do projeto (Hunter System)
- **Frontend:** React 19 + TypeScript, build com Vite 7, CSS Modules por componente.
- **Sync:** script Node standalone (`scripts/sync-notion.mjs`), sem framework, `fetch` nativo contra a API do Notion.
- **Gerenciador de pacotes:** npm (`package-lock.json` versionado).
- **Lint:** `npm run lint` (ESLint 9 + typescript-eslint).
- **Testes:** nenhuma suíte configurada ainda. Antes de aplicar TDD em qualquer feature nova, a primeira tarefa é instalar Vitest (e Testing Library para componentes) e definir o comando de teste aqui.
- **Deploy:** GitHub Actions, sem servidor próprio; `sync-notion.yml` roda a cada 15 minutos e `deploy.yml` publica no GitHub Pages a cada push na `main`.
- **Specs baseline:** `.docs/specs/baseline-sincronizacao-notion.md`, `.docs/specs/baseline-gamificacao.md`, `.docs/specs/baseline-interface.md`. Visão geral em `.docs/architecture.md`.
